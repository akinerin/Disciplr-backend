...
describe("Vault Cohort Retention Analytics View Tests", () => {
  // Create a clean mock database object using standard Jest functions
  ...
  it("asserts cohort retention math and formatting parameters are clean", async () => {
    // Pass our mockDb directly into the standalone function
    const result = await getCohortRetention(mockDb as any, 1);

    expect(result).toBeDefined();
    expect(result[0].cohort_month).toEqual("2026-05-01");
    expect(result[0].median_days_to_complete).toEqual(4.5);
    expect(result[0].completed).toEqual(5);
  });
});
